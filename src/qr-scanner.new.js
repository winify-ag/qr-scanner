...
export default class QrScanner {
    ...
    constructor(video, onDecode, canvasSize = QrScanner.DEFAULT_CANVAS_SIZE) {
        ...
        this._scanImageCallback = this._scanImageCallback.bind(this);
    ...
    /* async */
    static scanImage(imageOrFileOrUrl, options, callback) {
        ...
        //     }
            ...
            function onError(e) {
                ...
                callback('Scanner error: ' + errorMessage);
            }
    ...
    _scanFrame() {
        ...
        // requestAnimationFrame(() => {
            ...
            }, this._scanImageCallback)
                // .then(this._onDecode, error => {
                //     if (this._active && error !== 'QR code not found.') {
                //         console.error(error);
                //     }
                // })
                // .then(() => setTimeout(this._scanFrame.bind(this), 200));
        // });
    }

    _scanImageCallback(error, response) {
        if (error) {
            if (this._active && error !== 'QR code not found.') {
                console.error(error);
            }
        } else {
            this._onDecode(response)
        }
        scanImageTimeout = setTimeout(this._scanFrame, 100)
    }
